% Return MDH Parameter of the Atlas Robot Leg (v3) (hard coded)
% 
% Input:
% lr
...
function [a_mdh, d_mdh, alpha_mdh, q_offset_mdh, ...
  rSges_num_mdh, m_num_mdh, Iges_num_mdh] = atlas3_leg_parameter_mdh(lr)

assert(isa(lr,'logical') && all(size(lr) == [1 1]), ...
  'Left/Right flag has to be [1x1] logical');  

[p_num_urdf, rSges_num_urdf, m_num_urdf, Iges_num_urdf] = atlas3_leg_parameter_urdf(lr);
...
alpha_mdh(3) = pi/2;
a_mdh(3) = p_num_urdf(3);
phi = -atan(p_num_urdf(4)/p_num_urdf(5));
q_offset_mdh(3) = -phi;

a_mdh(4) = -sqrt(p_num_urdf(4)^2 + p_num_urdf(5)^2);
q_offset_mdh(4) = phi;

a_mdh(5) = p_num_urdf(6);

alpha_mdh(6) = -pi/2;
% a_mdh(6) = -p_num_urdf(6);
...
[rSges_num_mdh, Iges_num_mdh] = atlas3_leg_convert_urdf_mdh(lr, rSges_num_urdf, Iges_num_urdf);