% Return Parameter of the Atlas Robot (v4) (hard coded)
% 
% Input:
% lr
...
function [p_num, rSges_num, m_num, Iges_num] = atlas4_arm_parameter_urdf(lr)
...
% 1     shz: utorso -> clav
if lr == true % left
  p_num(1:3) = [0.1406, 0.2036, 0.4776];%0.2256, 0.4776];
...
% 2     shx: clav -> scap
if lr == true % left
  p_num(6:7) = [0.0981 -0.2511];%0.11, -0.245];
else
  p_num(6:7) = [-0.0981 -0.2511];%-0.11, -0.245];
end
...
if lr == true % left
  rSges_num(1,:) = [0, -0.00, -0.284]; % (1) clav
...
% TODO: Rotate inertia with rpy
% (1) clav
if lr == true % left
  % <origin xyz="0 -0.00 -0.284" rpy="-0.5 -0 0" /> 
  % <inertia ixx="0.011" ixy="0" ixz="0" iyy="0.009" iyz="-0.004" izz="0.004" />
  % The inertia is given in a different frame. Rotate inertia tensor into
  % the body frame
  I_inertiaframe = inertiavector2matrix([0.011, 0.009, 0.004, 0, 0, -0.004]);
  R_bodyframe_inertiaframe = rotx(0.5);
  I_bodyframe = R_bodyframe_inertiaframe*I_inertiaframe*R_bodyframe_inertiaframe';
  Iges_num(1,:) = inertiamatrix2vector(I_bodyframe);
else
  % <origin xyz="0 0.00 -0.284" rpy="0.5 -0 0" />
  % <inertia ixx="0.011" ixy="0" ixz="0" iyy="0.009" iyz="0.004" izz="0.004" />
