% Calculate Forward Kinematics for Atlas Robot Leg (v4)
% 
% Input:
% q [1x6]
%   Joint Angles [rad]
...
function T_c_urdf = atlas4_leg_fkine_num(q, lr)
%% Init
%#codegen
% Coder Information
...
% 
% 3     hpy: lglut -> uleg
T_urdf(:,:,3) = transl(p_num(2:4)) * troty(q(3));
% 
% 4     kny: uleg -> lleg
T_urdf(:,:,4) = transl([p_num(5), 0, p_num(6)]) * troty(q(4));
% 
% 5     aky: lleg -> talus
T_urdf(:,:,5) = transl([0,0,p_num(7)]) * troty(q(5));
% 
% 6     akx: talus -> foot
T_urdf(:,:,6) = trotx(q(6));
